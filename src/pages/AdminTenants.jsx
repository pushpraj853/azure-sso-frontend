  ...
  const save = async (e) => {
    ...
    const body = {
      ...
      allowedDomains: form.allowedDomains
        ? form.allowedDomains.split(",").map((s) => s.trim())
        : [],
    };
        ...
        >
          <form className="modal-card" onSubmit={save}>
            ...
            <div className="form-group">
              ...
              />
              <small>
                Comma-separated. Used to route dedicated SSO when the user enters an email on the
                login page.
              </small>
            </div>
            ...
            <div className="form-group">
              ...
              >
                ...
                <option value="dedicated">
                  Dedicated (client provides their own app registration)
                </option>
              </select>
            </div>
            ...
            {form.mode === "dedicated" && (
              <>
                <div className="form-group">
                  ...
                  />
                  <small>
                    The Application (client) ID from the client's Azure App Registration
                  </small>
                </div>
                <div className="form-group">
                  <label>
                    Client Secret{" "}
                    {editing && <span className="dim">(leave blank to keep existing)</span>}
                  </label>
                  <input
                    type="password"
      ...
      <div className="tenant-grid">
        {tenants.length === 0 && (
          <div className="empty-state">
            No client tenants configured yet. Add your first client above.
          </div>
        )}
        {tenants.map((t) => (
          <div className="tenant-card" key={t.id} style={{ borderTopColor: t.brandColor }}>
            <div className="tenant-card-header">
              ...
              <span className={`mode-badge ${t.mode}`}>
                {t.mode === "dedicated" ? "🔑 Dedicated" : "🌐 Standard"}
              </span>
            </div>
            <div className="tenant-meta">
              ...
              {t.clientId && (
                <div>
                  <span>Client ID</span>
                  <code>{t.clientId}</code>
                </div>
              )}
              {t.allowedDomains?.length > 0 && (
                <div>
                  <span>Domains</span>
                  <span>{t.allowedDomains.join(", ")}</span>
                </div>
              )}
              <div>
                <span>Added</span>
